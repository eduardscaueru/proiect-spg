...
void main()
{
    ...
    if (instance[0] < no_of_instances)
    {
        ...
        for (int line = 0; line < no_of_instances; line++) {
        for (float i = 0; i < no_of_generated_points; i++) {
            vec3 p1 = vec3(bezier(i/float(no_of_generated_points)));
            p1 = rotateY(p1, line * 0.5f);
            gl_Position = Projection * View * vec4(p1, 1);
            EmitVertex();
            vec3 p2 = vec3(bezier((i + 1)/float(no_of_generated_points)));
            p2 = rotateY(p2, line * 0.5f);
            gl_Position = Projection * View * vec4(p2, 1);
            EmitVertex();
            vec3 p22 = rotateY(p2, 0.5f);
            gl_Position = Projection * View * vec4(p22, 1);
            EmitVertex();
            EndPrimitive();
            ...
            gl_Position = Projection * View * vec4(p1, 1);
            EmitVertex();
            vec3 p11 = rotateY(p1, 0.5f);
            gl_Position = Projection * View * vec4(p11, 1);
            EmitVertex();
            EndPrimitive();
        }
    }
